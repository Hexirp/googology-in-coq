...
Definition 構築子@{ i | } : 単一型@{ i } := A_2024_07_21_0012@{ i } .

(** 単一型について場合分けします。 *)

Definition A_2024_08_28_0001@{ i | }
    : 単一型@{ i } -> forall 目標 : Type@{ i } , 目標 -> 目標
    := fun 対象 : 単一型@{ i } => fun 目標 : Type@{ i } => fun 処理 : 目標 => match 対象 with A_2024_07_21_0012 => 処理 end
.

Definition 場合分け@{ i | } ( 対象 : 単一型@{ i } ) ( 目標 : Type@{ i } ) ( 処理 : 目標 ) : 目標
    := A_2024_08_28_0001@{ i } 対象 目標 処理
.

(** 単一型について依存型の場合分けをします。 *)

Definition A_2024_08_28_0002@{ i | }
    : forall 対象 : 単一型@{ i } , forall 目標 : 単一型@{ i } -> Type@{ i } , 目標 構築子@{ i } -> 目標 対象
    :=
        fun 対象 : 単一型@{ i } =>
        fun 目標 : 単一型@{ i } -> Type@{ i } =>
        fun 処理 : 目標 構築子@{ i } =>
        match 対象 as 対象_ return 目標 対象_ with A_2024_07_21_0012 => 処理 end
.

Definition 依存型の場合分け@{ i | } ( 対象 : 単一型@{ i } ) ( 目標 : 単一型@{ i } -> Type@{ i } ) ( 処理 : 目標 構築子@{ i } ) : 目標 対象
    := A_2024_08_28_0002@{ i } 対象 目標 処理
.
...
Definition A_2024_08_28_0003@{ i | }
    ...
    := fun 前提 : Type@{ i } => fun 対象 : 前提 => 構築子@{ i }
.

Definition 余場合分け@{ i | } ( 前提 : Type@{ i } ) ( 対象 : 前提 ) : 単一型@{ i } := A_2024_08_28_0003@{ i } 前提 対象 .
