...
Definition A_2024_08_02_0006@{ i | } ( A : Type@{ i } ) ( B : Type@{ i } ) ( x : 双関数型@{ i } B A ) ( y : B ) : A
    := match x with A_2024_08_02_0005 _ _ x_1 x_2 => x_1 y end
.

(** [A] から [B] への関数と [B] から [A] への関数の直積の二番目の区域の値を取り出します。 *)

Definition A_2024_08_02_0007@{ i | } ( A : Type@{ i } ) ( B : Type@{ i } ) ( x : 双関数型@{ i } A B ) ( y : B ) : A
...
Definition A_2024_08_02_0009@{ i | } ( A : Type@{ i } )
    : 否定型@{ i } ( 双関数型@{ i } A ( 否定型@{ i } A ) )
    :=
        否定型.構築子@{ i }
            ( 双関数型@{ i } A ( 否定型@{ i } A ) )
            (
                let
                    a ( z : 双関数型@{ i } A ( 否定型@{ i } A ) ) : 空型@{ i }
                        :=
                            let
                                a : 否定型@{ i } A
                                    :=
